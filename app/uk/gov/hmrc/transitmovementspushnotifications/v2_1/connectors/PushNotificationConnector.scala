...
package uk.gov.hmrc.transitmovementspushnotifications.v2_1.connectors
...
class PushPullNotificationConnectorImpl @Inject() (appConfig: AppConfig, httpClientV2: HttpClientV2) extends PushPullNotificationConnector with BaseConnector {
  ...
  override def getBox(clientId: String)(implicit
    ...
    val url = appConfig.pushPullUrl.withPath(getBoxRoute).withQueryString(QueryString.fromPairs(("boxName", Constants.BoxNameFinal), ("clientId", clientId)))
